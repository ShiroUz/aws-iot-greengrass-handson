resource "aws_bedrock_inference_profile" "this" {
  name        = "weatherCheckLEDProfile"
  description = "Profile with tag for cost allocation tracking"

  model_source {
    copy_from = "arn:aws:bedrock:${data.aws_region.self.region}:${data.aws_caller_identity.self.account_id}:inference-profile/apac.anthropic.claude-3-haiku-20240307-v1:0"
